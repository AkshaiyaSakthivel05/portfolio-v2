...
const GRADIENT_BORDERS: Record<string, string> = {
  ...
  "from-amber-500 to-orange-500": "border-t-amber-500",
  "from-teal-500 to-emerald-500": "border-t-teal-500",
  "from-violet-500 to-purple-500": "border-t-violet-500",
  "from-rose-500 to-pink-500": "border-t-rose-500",
  "from-red-500 to-orange-500": "border-t-red-500",
  "from-sky-500 to-blue-500": "border-t-sky-500",
};
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6">
        ...
        <ScrollReveal className="text-center mb-12">
          ...
          <p className="text-slate-400 max-w-xl mx-auto">
            Production systems across AI trading, quantitative finance, and full-stack engineering.
          </p>
        </ScrollReveal>
                    ...
                    <div className="relative z-10">
                        ...
                        {project.category.map((cat) => {
                          ...
                          return (
                            <Badge
                              key={cat}
                              variant="secondary"
                              className="bg-indigo-500/15 text-indigo-300 border border-indigo-500/25 text-xs gap-1"
                            >
                              <Icon size={10} />
                              {cat}
                            </Badge>
                      ...
                      <div className="flex flex-wrap gap-1.5 mb-5">
                        {project.tech.map((t) => (
                          <span
                            key={t}
                            className="px-2 py-0.5 text-xs rounded-md bg-slate-800 text-slate-400 border border-slate-700"
                          >
                            {t}
                          </span>
                        ))}
                      ...
                      <div className="relative z-10">
                        ...
                        <div className="flex flex-wrap gap-1.5 mb-3">
                          {project.category.map((cat) => (
                            <Badge
                              key={cat}
                              variant="secondary"
                              className="bg-slate-800 text-slate-400 border border-slate-700 text-xs"
                            >
                              {cat}
                            </Badge>
                          ))}
                        ...
                        <div className="flex flex-wrap gap-1 mb-4">
                          {project.tech.slice(0, 4).map((t) => (
                            <span
                              key={t}
                              className="px-1.5 py-0.5 text-xs rounded bg-slate-800/80 text-slate-500"
                            >
                              {t}
                            </span>
                          ))}
